...
| `--group-by-team-prefix <prefixes>`  | string                            | ❌       | `""`               | Team-name prefixes for grouping result repos by GitHub team, as a hierarchy. `/` nests levels within one chain (`tribe-/squad-`), `,` separates independent chains (`tribe-/squad-,chapter-`). Requires `read:org` scope. See [Team grouping](/usage/team-grouping).                                                                                                                              |
| `--exclude-team-prefixes <prefixes>` | string                            | ❌       | `""`               | Comma-separated team-name prefixes to exclude from grouping entirely, before combined sections are formed. Useful to filter out noisy/overly granular team names. A repo left with no matching team after exclusion falls into `other`. Only applies with `--group-by-team-prefix`. See [Excluding noisy team prefixes](/usage/team-grouping#excluding-noisy-team-prefixes).                      |
